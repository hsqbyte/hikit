    ...
    const calcHash = async () => {
        ...
        for (const a of ['SHA-1', 'SHA-256', 'SHA-512']) {
            try {
                ...
                newResults.push({ algo: a, value: Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join('') });
            } catch {
                newResults.push({ algo: a, value: '不支持' });
            }
        }
    ...
    return (
        <>
            ...
            <textarea className="tool-textarea" value={input} onChange={(e) => setInput(e.target.value)}
                placeholder="输入要计算 Hash 的文本..." spellCheck={false} />
                    ...
                    {results.map((r) => (
                        <div key={r.algo} className="hash-result-item">
                            ...
                            <Button size="small" type="text" icon={<CopyOutlined />}
                                onClick={() => { navigator.clipboard.writeText(r.value); message.success('已复制'); }} />
                        </div>
                    ))}
                </div>
            )}
